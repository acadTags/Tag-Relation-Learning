 ...
 * @author Hang Dong
 */
public class MappingACMhierarchyToTags {
    public static void main(String [] args){
        HashMap<String,String> dbo_redir = LoadRedirectsData("acm_data_equivalence_reduced.txt");
        
        MappingToTags("äcm_preflabel_hierarchy.txt",MapDBpediaConceptToTag("äcm_preflabel_hierarchy.txt",dbo_redir,"taglist_tf3_new.csv",1));
        ...
        try{
            ...
            while ((tempString = readerRedirect.readLine()) != null) {
                ...
                if (concepts.length == 2){
                    ...
                    if (dbo_redir.containsKey(concepts[0])){
                        dbo_redir.put(concepts[0], dbo_redir.get(concepts[0]) + " ||| " + concepts[1]);
                    }else{
                        dbo_redir.put(concepts[0], concepts[1]);
                    }
