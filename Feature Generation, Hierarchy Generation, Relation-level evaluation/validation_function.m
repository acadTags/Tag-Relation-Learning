...
% the output ret is [recall,precision,f-score,accuracy,roc_auc].
labels = (labels >= 0) - (labels < 0);
%valid_function = @(dec, labels) recall(dec, labels);
ret = [recall(dec, labels),precision(dec, labels),fscore(dec, labels),accuracy(dec, labels),auc(dec, labels)];
