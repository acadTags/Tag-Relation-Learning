...
fileNameSuffix = 'data';
%outputNameSuffix = ['s1_dbpedia' '_ada'];
...
trainedClassifier = trainClassifierAdaboostFtSelection(featureTable_all,feature_selection_set);
...
for ith_tag = 1:num_seeds
    ...
    [prediction,score] = predict(model,feature_matrix_prediction(:,feature_selection_set)); % need also to select the same features for the prediction data 
    ...
    while (size(candidate_tag_list,1)>=num_stop)
        ...
        [ next_level_relations,next_level_relations_score,candidate_tag_list ] = predictNextLevelRelationsRevForEnrichmentWithAdaboost_ft_select(model,current_level_relations,candidate_tag_list,taglist,pzt,ptz,pz,minF,maxF,threshold,sim_threshold,p_sig_topics,feature_selection_set);
