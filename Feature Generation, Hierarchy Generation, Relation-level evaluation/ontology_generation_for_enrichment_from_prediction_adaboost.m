% ontology generation for enrichment from prediction with adaboost
%clearvars;
...
%load('seed_tags_to_predict_dbpedia_education_3.mat')
%seed_tags_with_root = seed_tags_to_predict_dbpedia_education_3;
...
%load('bibsonomy_hier_instances_final_0.001_features_alex8ft.mat');clear cell;
...
% set [posterior] confidence threshold.
confidence_threshold_posterior = 0;
%confidence_threshold_posterior = 0.65;

outputNameSuffix = ['14ft' '_ada_' num2str(confidence_threshold_posterior)];

% generate training data and train the model
trainedClassifier = trainClassifierAdaboost(featureTable_all);
...
for ith_tag = 1:num_seeds
    ...
    first_level_relations = random_tag_list_for_prediction(strcmp(prediction,'b'),:);
    first_level_relations_score = score(strcmp(prediction,'b'),1);
    ...
    while (size(candidate_tag_list,1)>=num_stop)
        ...
        [ next_level_relations,next_level_relations_score,candidate_tag_list ] = predictNextLevelRelationsRevForEnrichmentWithAdaboost(model,current_level_relations,candidate_tag_list,taglist,pzt,ptz,pz,minF,maxF,threshold,p_sig_topics);
    ...
    %cd 'LO'
    %cell2csv([tag1 ' ' outputNameSuffix file_name_ending],all_relations(:,1:2));
    if (size(all_relations_filtered,1) ~= 0)
        cell2csv([tag1 ' ' outputNameSuffix file_name_ending],all_relations_filtered(:,1:2));
    else
        cell2csv([tag1 ' ' outputNameSuffix file_name_ending],all_relations_filtered);
    end
    %cd ..
