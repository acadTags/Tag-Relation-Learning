...
fileNameSuffix = 'data';
%outputNameSuffix = 's1_fine_dbpedia';
prob_estimate = false;
%cvalue = 2^10;
%gammavalue = 2^11;
weight = 1;
...
% feature_selection_set = set_topic_dist_2;
...
training_data = [label_training,feature_matrix(:,logical(feature_selection_set))]; % this is min-maxed.
...
testing_data = [label_validation,feature_matrix_validation(:,logical(feature_selection_set))]; % this is min-maxed.
...
for ith_tag = 1:num_seeds
    ...
    fakeY = ones(m,1);
    prediction_data = [fakeY,feature_matrix_prediction(:,logical(feature_selection_set))];
    ...
    while (size(candidate_tag_list,1)>=num_stop)
        ...
        [ next_level_relations,next_level_relations_score,candidate_tag_list ] = predictNextLevelRelationsRevForEnrichment_ft_select(model,current_level_relations,candidate_tag_list,taglist,pzt,ptz,pz,minF,maxF,threshold,sim_threshold,p_sig_topics,prob_estimate,feature_selection_set);
